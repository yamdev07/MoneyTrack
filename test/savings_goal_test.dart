...
void main() {
  group('SavingsGoal', () {
    test('progress is the saved/target ratio, clamped to 1', () {
      ...
      final over = SavingsGoal(label: 'PC', targetAmount: 100, savedAmount: 150);
      expect(over.progress, 1.0);
    });

    test('remaining never goes below zero', () {
      final goal = SavingsGoal(label: 'PC', targetAmount: 100, savedAmount: 120);
      expect(goal.remaining, 0);
    });

    test('isReached when saved meets the target', () {
      final goal = SavingsGoal(label: 'PC', targetAmount: 100, savedAmount: 100);
